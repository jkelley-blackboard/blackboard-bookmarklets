javascript:(async function() {
  ...
  const parseRow = row => {
    ...
    const toolName = nameInput ? nameInput.value : row.querySelector("th")?.innerText.trim();
    const vendor = row.querySelector("td:nth-of-type(2) .table-data-cell-value")?.innerText.trim();
    const version = row.querySelector("td:nth-of-type(3) .table-data-cell-value")?.innerText.trim();
    const supportStatus = row.querySelector("td:nth-of-type(4) .table-data-cell-value")?.innerText.trim();
  ...
  const ensurePanel = doc => {
    ...
    panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
        <span style="font-weight:600">B2 Export Utility</span>
        <button id="cmpClose" class="cmp-btn" style="background:#111;color:#fff;padding:2px 8px;border-radius:6px">✕</button>
      </div>
  ...
  renderPageContext(doc, panel, ctx);
