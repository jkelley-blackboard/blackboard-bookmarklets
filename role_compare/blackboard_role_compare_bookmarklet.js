(() => {
  ...
  // 3. Styles (minimized for bookmarklet safety)
  ...
  panel.innerHTML = '<div id="paginationWarning"></div><div id="roleDetails"></div><div id="buttonContainer"></div><hr>';
  doc.body.appendChild(panel);
  makeDraggable(panel);
  panel.style.left = ((window.innerWidth - panel.offsetWidth) / 2) + "px";

  // 6. URL params & warnings
  ...
  warnDiv.innerHTML = allShown ? "" : "⚠ Click Show All before comparing.";
  if (!allShown) return;
  ...
  // 8. Refresh frame button
  addButton("Refresh Frame", () => {
    iframe.contentWindow.location.reload();
  });

  // 9. Upload JSON button
    ...
    input.onchange = e => {
      ...
      reader.onload = ev => {
        try {
          ...
          infoDiv.innerHTML = 
            '<b>Role Type:</b> ' + data.roleType +
            '<br><b>System:</b> ' + data.system +
            '<br><b>Role:</b> ' + data.role +
            '<br><b>Date:</b> ' + data.timestamp +
            '<div id="filterButtons">' +
            '<button id="filterMismatch">Mismatches</button>' +
            '<button id="filterPermitted">Permitted</button>' +
            '<button id="filterRestricted">Restricted</button>' +
          ...
          rows.forEach(tr => {
            ...
            const actualStatus = imgSrc.includes("checkmark") ? "permitted" :
                                 imgSrc.includes("dash") ? "inherited" : "restricted";

            if (name && data.privileges[name]) {
              const expectedStatus = data.privileges[name].status;
              if (expectedStatus !== actualStatus) {
                const statusCell = tr.querySelector("td:nth-child(2)");
                if (statusCell) {
                  const warnIcon = doc.createElement("span");
                  warnIcon.textContent = "⚠️";
                  warnIcon.title = "Mismatch: expected " + expectedStatus + ", actual " + actualStatus;
                  warnIcon.style.fontSize = "18px";
                  warnIcon.style.marginLeft = "6px";
                  warnIcon.style.color = "orange";
                  statusCell.appendChild(warnIcon);
                }
              }
            }
          });
          ...
          const filterRows = (mode) => {
            rows.forEach(tr => {
              ...
              const actualStatus = imgSrc.includes("checkmark") ? "permitted" :
                                   imgSrc.includes("dash") ? "inherited" : "restricted";
              const expectedStatus = data.privileges[name]?.status;
              const mismatch = expectedStatus && expectedStatus !== actualStatus;

              if (mode === "mismatch") tr.style.display = mismatch ? "" : "none";
              else if (mode === "permitted") tr.style.display = expectedStatus === "permitted" && actualStatus !== "permitted" ? "" : "none";
              else if (mode === "restricted") tr.style.display = expectedStatus === "restricted" && actualStatus !== "restricted" ? "" : "none";
              else tr.style.display = "";
            });
          };

          doc.getElementById("filterMismatch").onclick = () => filterRows("mismatch");
          doc.getElementById("filterPermitted").onclick = () => filterRows("permitted");
          doc.getElementById("filterRestricted").onclick = () => filterRows("restricted");
          doc.getElementById("filterAll").onclick = () => filterRows("all");
