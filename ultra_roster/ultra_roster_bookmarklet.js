...
(async function () {
  ...
  try {
    ...
    function buildPrintCardsHtml(rows) {
      return rows.map(row => `
        <div class="bb-card">
          <img class="bb-card-avatar" src="${esc(row.avatarSrc)}" alt="${esc(row.fullName)}">
          <div class="bb-card-name">${esc(row.fullName)}</div>
          <div class="bb-card-role">${esc(row.role)}${row.pronouns ? ' • ' + esc(row.pronouns) : ''}</div>
          <div class="bb-card-email">${esc(row.email)}</div>
        </div>`).join('');
    }

    function buildPrintWindowHtml() {
      const printRowsHtml = buildPrintRowsHtml(rosterRows);
      const printCardsHtml = buildPrintCardsHtml(rosterRows);
      const printDate = new Date().toLocaleString();
      return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>${esc(courseTitle)} — Roster Print</title><style>body{font-family:Arial,sans-serif;margin:0;padding:20px;color:#111}h1{margin:0 0 8px;font-size:20px}p{margin:0 0 16px;color:#666;font-size:12px}table{width:100%;border-collapse:collapse;font-size:11px}th,td{border:1px solid #ddd;padding:6px 8px;text-align:left;vertical-align:top}th{background:#f5f5f5;font-weight:700}tr:nth-child(even){background:#fafafa}.bb-print-avatar{width:24px;height:24px;border-radius:50%;object-fit:cover;display:block}.bb-cards{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:12px}.bb-card{border:1px solid #ddd;border-radius:6px;padding:10px;text-align:center;break-inside:avoid;page-break-inside:avoid}.bb-card-avatar{width:64px;height:64px;border-radius:50%;object-fit:cover;margin:0 auto 8px;display:block}.bb-card-name{font-weight:700;font-size:12px;margin-bottom:2px;word-break:break-word}.bb-card-role{font-size:10px;color:#666;margin-bottom:4px}.bb-card-email{font-size:9px;color:#888;word-break:break-word}.toolbar{display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:16px}.view-toggle button{padding:6px 12px;border:1px solid #1d3557;background:#fff;color:#1d3557;cursor:pointer;font-size:12px}.view-toggle button:first-child{border-radius:4px 0 0 4px}.view-toggle button:last-child{border-radius:0 4px 4px 0}.view-toggle button.active{background:#1d3557;color:#fff}.print-actions{display:flex;gap:8px}.print-actions button{padding:8px 14px;border:none;border-radius:4px;background:#1d3557;color:#fff;cursor:pointer}@media print{body{padding:0}.toolbar{display:none}}</style></head><body><div class="toolbar"><div class="view-toggle"><button id="bbViewTableBtn" class="active" onclick="bbSetView('table')">📋 Table</button><button id="bbViewCardsBtn" onclick="bbSetView('cards')">🖼 Cards</button></div><div class="print-actions"><button onclick="window.print()">🖨 Print</button><button onclick="window.close()">✖ Close</button></div></div><h1>${esc(courseTitle)}</h1><p>${rosterRows.length} enrolled users • Printed ${esc(printDate)}</p><div id="bbTableView"><table><thead><tr><th>Avatar</th><th>Name</th><th>Username</th><th>Email</th><th>Student ID</th><th>Other Name</th><th>Pronouns</th><th>Pronunciation</th><th>Role</th><th>Availability</th><th>Enrollment Date</th><th>Last Login</th><th>Last Accessed</th></tr></thead><tbody>${printRowsHtml}</tbody></table></div><div id="bbCardsView" class="bb-cards" style="display:none">${printCardsHtml}</div><script>function bbSetView(v){document.getElementById('bbTableView').style.display=v==='table'?'':'none';document.getElementById('bbCardsView').style.display=v==='cards'?'':'none';document.getElementById('bbViewTableBtn').classList.toggle('active',v==='table');document.getElementById('bbViewCardsBtn').classList.toggle('active',v==='cards')}</script></body></html>`;
    }
